...
const containsPackage = (files, package) =>
  files.some(file => extractPackage(file) === package)
...
module.exports = {
  '*.{j,t}s?(x)': files =>
    [
      ...
      containsPackage(files, 'packages/client/') &&
        'jest --ci --forceExit packages/client/src/storyshots.test.ts',
    ].filter(Boolean),
  'packages/*/**/*.ts?(x)': files =>
