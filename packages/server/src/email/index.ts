...
let sendVerification: (data: {
  ...
  throw new Error(
    'email verification requested when email provider is not configured'
  )
}
...
if (emailConfig) {
  const provider = (async () => {
    const { default: Provider } = (await import(
      path.join('../providers', emailConfig.provider.name)
    )) as { default: ProviderConstructor }
    return new Provider(emailConfig.provider.options ?? {})
  })()

  const verifyHtml = fs
    .readFileSync(path.join(__dirname, 'emails/verify.html'))
    .toString()
  const verifyText = fs
    .readFileSync(path.join(__dirname, 'emails/verify.txt'))
    .toString()
